...
const Input = ({ type = "text", value: defaultValue, ...restProps }) => {
	...
	labelClassName += isLabelOnTop ? " top-0 left-2 px-2 text-sm bg-white" : " top-1/2 left-[4.25rem] text-lg";

	let inputClassName = "w-full p-4 pl-[4.25rem] text-lg border border-solid border-gray-200 rounded-md";
	...
	return (
		<div className="relative w-full my-8 rounded-md">
			{restProps.icon && (
				<FontAwesomeIcon icon={restProps.icon} className="absolute top-1/2 left-4 -translate-y-1/2 w-9 text-3xl text-gray-300" />
			)}
			{type === "password" && (
				<FontAwesomeIcon
